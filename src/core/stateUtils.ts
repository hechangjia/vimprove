...
export const createSnapshot = (state: VimState): VimState => ({
  ...
  pendingZ: state.pendingZ,
  pendingCtrlW: state.pendingCtrlW,
  ...
  currentBufferIndex: state.currentBufferIndex,
  windows: state.windows.map(window => ({ ...window })),
  currentWindowIndex: state.currentWindowIndex,
  quickfixList: state.quickfixList.map(item => ({ ...item })),
  quickfixIndex: state.quickfixIndex,
  quickfixOpen: state.quickfixOpen,
  history: [],
  historyIndex: -1,
